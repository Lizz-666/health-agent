...
void main() {
  ...
  testWidgets('not-configured shows missing state, readiness and missing fields',
      (tester) async {
    final adapter = FakeDioAdapter()
      ..registerJson(
        'GET',
    ...
    expect(find.textContaining('fitness_goal'), findsOneWidget);
  ...
  testWidgets('restricted readiness shown explicitly, never as ready', (
    ...
    final adapter = FakeDioAdapter()
      ..registerJson(
        ...
        (_) => _result(
          true,
          _profile,
          _readiness('restricted', restricted: '安全筛查受限'),
        ),
      );
    await tester.pumpWidget(_wrap(_apiWith(adapter)));
    await tester.pumpAndSettle();
    ...
    expect(find.textContaining('安全筛查受限'), findsOneWidget);
  ...
  testWidgets('edit flow opens form and saves with lists preserved', (
    ...
    expect(
      (body['pain_injury_limitations'] as List).first['body_area'],
      '下背',
    );
  ...
  testWidgets('network error shows retry and not empty/ready', (tester) async {
    final adapter = FakeDioAdapter()
      ..registerError(
        ...
        '/health/profile',
        503,
        {'detail': '健康档案加载失败', 'code': 'service_unavailable'},
      );
    await tester.pumpWidget(_wrap(_apiWith(adapter)));
    await tester.pumpAndSettle();
  ...
  testWidgets(
    'logout/clearing: provider reset drops stale health UI values',
    (tester) async {
      final adapter = FakeDioAdapter()
        ..registerJson(
          'GET',
