...
3. **规则守底线，AI 做个性化**：确定性规则负责风险、禁忌、计算和校验；AI 负责理解、组合、解释和交互。
...
- 多项问题并存时，Agent 根据关联资料和规则提出改善优先级
...
Agent 可以自动执行当日缩短、顺延或恢复性替换并告知用户；改变每周结构、长期强度或主要目标时必须由用户确认。出现明确安全风险时，可自动停止相关动作。
...
Agent 通过受控 Tools 工作，不直接任意读写数据库。
