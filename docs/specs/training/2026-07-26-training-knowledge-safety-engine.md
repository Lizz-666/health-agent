...
- Codex owns and locks safety-policy tables, state precedence, normalization behavior, and executable safety cases before OpenCode implements Tasks 4-6. OpenCode does not choose or relax safety behavior. Any unspecified safety decision pauses implementation for Codex to resolve; Codex directly takes over the affected change when review cannot reduce it to a mechanical implementation.
