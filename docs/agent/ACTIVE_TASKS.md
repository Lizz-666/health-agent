...
- 阶段：Phase 8 个人开发版验收，状态 `in_progress`。Gate 1 closure `3de6d3f1b47b6403e321710b4216eeb81636aa6a` 已通过本地 Fast 927/0/19 和 exact-SHA CI `31305119808`：Fast 936/0/10、Flutter 472、Full 1687/0/0、PostgreSQL 28/28，三项全绿。Gate 2 仅在同一实现分支/worktree 由 Codex 单写入。
...
| Phase 8 Task 2 | Codex sole real-HTTP runner, evaluation, CI, cold reviewer, and Gate 2 acceptor | `codex/phase8-implementation` / `health-worktrees/phase8-implementation` | `3de6d3f1b47b6403e321710b4216eeb81636aa6a` | `in_progress` | 允许 `scripts/phase8.py`、`backend/tests/phase8_{device_server,loopback}.py`、`backend/tests/test_phase8_{http_journey,evaluations}.py`、两个 `backend/tests/data/phase8_*_cases.v1.json`、`scripts/verify.py`、`backend/tests/test_{verify_runner,ci_action_pins}.py`、`.github/workflows/ci.yml`、focused run 文档，以及冷审证明 Windows IANA fallback 必需的 `backend/requirements.txt` 单行 `tzdata` pin。真实 HTTP 揭示的 Phase 7 回顾草稿不可创建/确认缺口由 Codex 接管，额外允许 `backend/app/training/{service,review_service,schemas_api}.py`、`backend/app/agent/action_tools.py`、`backend/tests/phase8_fixtures.py`、`backend/tests/test_phase8_harness.py`、`backend/tests/test_training_weekly_reviews.py`、`backend/tests/test_{training_api,agent_confirmation,agent_orchestrator,phase4_e2e,phase6_e2e,phase7_e2e}.py`，以及 `app/lib/models/plan.dart`、`app/lib/screens/plan/plan_screen.dart` 和对应 model/provider/screen 测试；契约澄清同步到 Phase 7 weekly-review spec 与 ADR-0007。修复仅允许服务端回顾提案的相邻规划偏好适配、latest-review stale 复核与 exact draft 确认，不改变健康阈值或普通草稿安全门。目标是真实 localhost/JWT journey、版本化安全/Agent eval、fail-fast CLI 与非 live CI job。Task 3/OpenCode 在 Gate 2 exact-SHA 接受前禁止启动。 |
