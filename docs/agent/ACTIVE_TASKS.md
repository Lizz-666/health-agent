...
| Phase 1 Task 8 | OpenCode (impl) + Codex review | `codex/phase1-task8-flutter-posture-profile` / `health-worktrees/phase1-task8-flutter-posture-profile` | `2f4551e` | `planned` | Flutter 服务端体态档案、冲突/不确定状态、优先级三路分区、1–3 目标确认和结构化安全信号入口；复用现有 `/profile/posture` 路由并替换本地状态占位页。不得触碰 `starlit-galaxy-rolls-21h55` 或与其重叠的 app.dart/constants/theme/home/issues/widgets；不实现训练计划或 Agent UI。客户端副作用请求使用稳定 UUID 幂等键，stale_priority 和安全信号成功后必须刷新 priorities，不把网络/解析失败渲染成空档案或 normal |
