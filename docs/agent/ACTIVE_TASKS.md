...
| Phase 9 Task 3B | Codex sole Android reliability/E2E writer and Gate 3 acceptor | `codex/phase9-implementation` / `C:\Users\Lenovo\Desktop\develop\health-worktrees\phase9-implementation` | Gate 3A exact closure `a3c8b5a5e4fb4f8129a397c1593185bdbf346a87` | `in_progress` | 冻结矩阵：Android API 34 + API 28；冷启动/后台恢复；受控试用激活、独立账号/设备、401 单次并发 refresh、撤销/过期清态；慢响应、断网/不可达、显式重试和重复提交；`Asia/Shanghai` 跨午夜/周界；客户端版本头与服务端 `426` fail-closed；`0013 -> 0014/head` 升级及服务关闭状态 downgrade/re-upgrade 恢复演练。生产代码 allowlist 仅 `backend/app/{main.py,core/config.py,core/compatibility.py}`、`app/lib/core/{constants.dart,api_client.dart}`、`app/lib/providers/auth_provider.dart`、`app/lib/screens/auth/login_screen.dart`；测试/runner 仅对应 `backend/tests/{phase8_app_factory.py,phase9_*,test_client_compatibility.py,test_trial_privacy.py}`、`app/test/{core/api_client_test.dart,providers/auth_session_reset_test.dart,screens/login_screen_test.dart}`、`app/integration_test/phase9_*`、`scripts/{phase9.py,verify.py}`、`.github/workflows/ci.yml`；治理证据仅当前计划、`ACTIVE_TASKS`、Phase9 运维/Android matrix/Gate 3 报告。禁止 migration、模型、健康/安全规则、live/photo、依赖和其他业务界面；任何扩围先记录 finding。只用合成数据，不使用真实凭据。 |
