...
| Phase 1 Task 10.5 | OpenCode (impl) + Codex review | `codex/phase1-task10-5-migration-contract` / `health-worktrees/phase1-task10-5-migration-contract` | `49f9a8d` | `planned` | Migration Phase C contract/cleanup：0003 收紧 source/lifecycle NOT NULL，severity 永久 nullable，删除数据库 method/result 旧列并移除 dual-write；API method/result 兼容字段继续由 source/severity 映射。必须完成 PostgreSQL 16 upgrade/downgrade/re-upgrade 与普通/null severity 数据往返；Task 10 最终 E2E 在本任务合入前不得启动 |
