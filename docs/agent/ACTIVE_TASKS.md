...
| Phase 1 Task 8A | OpenCode (impl) + Codex review | `codex/phase1-task8a-history-source` / `health-worktrees/phase1-task8a-history-source` | `55acc62` | `planned` | 后端 history 兼容补丁：每条记录新增真实 `source`，保留同值 `method` 别名；仅限 schemas/service/test_posture，不建 migration、不改变写入/投影/风险逻辑；与 8B 文件不重叠，可并行 |
| Phase 1 Task 8B | OpenCode (impl) + Codex review | `codex/phase1-task8b-flutter-foundation` / `health-worktrees/phase1-task8b-flutter-foundation` | `55acc62` | `planned` | Flutter 模型、Provider、API 状态与稳定 UUID 幂等基础层；明确区分有效空数据与失败，未知关键枚举不得降级为 normal；不修改任何 screen/route/theme/home/issues/widgets；与 8A 并行 |
