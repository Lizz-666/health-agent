...
| Phase 4 Tasks 0-7 | OpenCode + selected Claude model; Codex final acceptance only | `codex/phase4-opencode-implementation` / `health-worktrees/phase4-opencode-implementation` | coordination baseline pending | `planned` | Follow `docs/agent/PHASE4_OPENCODE_CHARTER.md`. One writer/session; OpenCode owns spec through self-acceptance and may commit/push/update one draft PR. It must stop at final `review` handoff; only Codex may mark verified or integrate. |
